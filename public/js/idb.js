...
// This function will be executed if we attempt to submit a new trans and there's no internet connection
        ...
        if (getAll.result.length > 0) {
            fetch('/api/transaction', {
                ...
                .then(serverResponse => {
                    ...
                    const transObjectStore = transaction.objectStore('new_trans');
                    // clear all transs in your store
                    transObjectStore.clear();

                    alert('All saved trans has been submitted!');
                })
                .catch(err => {
                    console.log(err);
                });
